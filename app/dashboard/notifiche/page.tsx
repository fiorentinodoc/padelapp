...
export default function NotifichePage() {
  ...
  async function loadManualCandidates(target: string) {
    ...
    if (target !== 'all') query = query.eq('level', target)
  ...
  function openAllWhatsApp(cands: Candidate[], message: string) {
    cands.forEach((c, i) => {
      if (c.phone) setTimeout(() => window.open(buildWhatsAppLink(c.phone!, message), '_blank'), i * 800)
    })
  }
    ...
    <div style={{ padding: isMobile ? '20px 16px' : '32px', fontFamily: 'system-ui', color: text, background: bg, minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        <div style={{ fontSize: '15px', fontWeight: '700', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '12px', background: cancellations.length > 0 ? 'rgba(232,88,88,0.15)' : surface2, color: cancellations.length > 0 ? '#e85858' : textMuted, padding: '2px 8px', borderRadius: '20px', fontWeight: '600' }}>
            {cancellations.length}
          </span>
        </div>

        {cancellations.length === 0 ? (
          <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: '14px', padding: '32px 20px', textAlign: 'center', color: textMuted, fontSize: '14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {cancellations.map(canc => {
              ...
              return (
                <div key={canc.booking_id} style={{ background: surface, border: `1px solid ${border}`, borderRadius: '16px', padding: '20px' }}>
                  ...
                  <div style={{ background: surface2, borderRadius: '10px', padding: '12px 14px', marginBottom: '14px', fontSize: '13px', color: textSub, whiteSpace: 'pre-line', lineHeight: '1.5' }}>
                    {message}
                  </div>

                  {cands.length === 0 ? (
                    <div style={{ fontSize: '13px', color: textMuted, textAlign: 'center', padding: '8px' }}>
                      Nessun alunno disponibile con numero di telefono
                    </div>
                  ) : (
                    <>
                      <div style={{ fontSize: '12px', fontWeight: '700', color: textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
                        Alunni compatibili ({levelLabel[canc.level]}) — {cands.length} con telefono
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px' }}>
                        {cands.map(cand => (
                          <div key={cand.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: surface2, borderRadius: '10px', padding: '10px 14px' }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: '14px', fontWeight: '600', color: text }}>{cand.first_name} {cand.last_name}</div>
                              <div style={{ fontSize: '12px', color: textMuted }}>{cand.phone}</div>
                            </div>
                            <button onClick={() => openWhatsApp(cand.phone!, message)}
                      ...
                      {cands.length > 1 && (
                        <button onClick={() => openAllWhatsApp(cands, message)}
      ...
      <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: '16px', padding: '24px' }}>
        ...
        <div style={{ marginBottom: '14px' }}>
          <label style={{ fontSize: '11px', fontWeight: '700', color: textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '6px' }}>Destinatari</label>
          <select value={manualForm.target}
            onChange={async e => {
        ...
        <div style={{ marginBottom: '16px' }}>
          <label style={{ fontSize: '11px', fontWeight: '700', color: textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: '6px' }}>Messaggio</label>
          <textarea value={manualForm.message}
            onChange={e => setManualForm(f => ({ ...f, message: e.target.value }))}
            placeholder="Es: Lezione di sabato spostata alle 11:00 ⚠️"
            rows={4}
            style={{ ...selectStyle, resize: 'vertical', fontFamily: 'system-ui' }} />
        </div>
          ...
          <div style={{ marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto' }}>
              {manualCandidates.map(cand => (
                <div key={cand.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: surface2, borderRadius: '8px', padding: '8px 12px' }}>
                  <div style={{ flex: 1, fontSize: '13px', color: text }}>
                    {cand.first_name} {cand.last_name} · <span style={{ color: textMuted }}>{cand.phone}</span>
                  </div>
                  <button onClick={() => openWhatsApp(cand.phone!, manualForm.message)}
        ...
        <button
          onClick={() => {
            ...
            if (manualCandidates.length === 0) { loadManualCandidates(manualForm.target); return }
            openAllWhatsApp(manualCandidates, manualForm.message)
          }}
          disabled={!manualForm.message.trim()}
          style={{ width: '100%', background: manualForm.message.trim() ? '#25D366' : surface2, border: 'none', color: manualForm.message.trim() ? '#fff' : textMuted, padding: '13px', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: manualForm.message.trim() ? 'pointer' : 'not-allowed' }}>
